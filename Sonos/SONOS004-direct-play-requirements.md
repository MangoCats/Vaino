...
**`[GDE-SONOS-340]` The encoder is the only capability Vaino does not already have.** `PassageDecoder` and the engine's own mixer already produce f32 PCM continuously for `cpal`; Sonos's `CurrentURI` needs a widely-playable streamed format, and PCM/WAV is not it. Two candidates were weighed — `mp3lame-encoder` (Rust bindings over libmp3lame) and `shine` (a fixed-point C encoder with no existing Rust binding at all) — compared in full in [SONOS005](SONOS005-encoder-choice.md), which the choice below already assumes.
