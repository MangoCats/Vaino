...
`[Sonos/SONOS010]` item 6 -- "never run end-to-end against the real Office pair" -- run for the first time this weekend, at the user's own initiative, across several sessions as each finding led to the next. Eight real issues found; six fixed and confirmed live, one (§6, periodic silence) fixed once, found still present, and fixed again against a proven-working reference -- not yet reconfirmed -- and the last (§7, skip lag) diagnosed to a specific cause and deliberately deferred rather than folded into an already-large session of changes to the same sensitive code. This document is additive: nothing in `[SONOS002]`, `[SONOS008]`, `[SONOS009]`, `[SONOS010]`, or `[SONOS011]` is wrong so much as untested against a condition none of them anticipated -- a local output device that is not briefly absent but **entirely, indefinitely absent** for the whole session.
...
**A rating, since accuracy varied enough this session to be worth stating plainly, per the question that sent this table back into the document:**

| Source | Kind | Result this session |
|---|---|---|
| Own trained knowledge (`x-rincon-mp3radio://`, `audioBroadcast`) | recalled, not looked up | Correct and load-bearing (§2) -- fixed the first `500` outright. |
...
| Generic Sonos Community forum thread, via `WebSearch` synthesis | secondary, summarised | **Directionally right, materially incomplete.** Correctly named a real chunked-transfer/MP3 incompatibility and the shape of the fix; gave no working number, and an invented one did not resolve the symptom. Worth treating as "what to try," never as "what value to use," without a primary source alongside it. |
| Music Assistant `streams/controller.py` (GitHub, primary, **proven on this hardware**) | live fetch | The most trustworthy source available to this project for anything Sonos-specific: not a claim about Sonos in general, but the actual working configuration for the actual speaker in the room. Where it and a forum thread disagree, or a forum thread is silent on specifics, this is the one to follow. |
