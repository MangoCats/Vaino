...
This designs against `[REQ-LIB-180]`. *(Requested 2026-08-27; designed and built 2026-08-27 — recording/artist search and artist-only correction; release and track search remain designed, not built, per `[SPEC-SUI-197]`'s table below.)* The review queue above settles the case AcoustID can speak to. It cannot help with the others, and there are three of them: self-released audio with no fingerprint entry at all, a remaster or bootleg AcoustID has never indexed, and — a case the queue does not even see today — a recording id that is *right* while its **credited artist** is wrong. `unverified` alone is 866 passages on this library, none of them reachable by a radio button.
...
>
> **Built in Vaino, not Sampo's console — a correction to this paragraph's first draft.** `/review` is served by Vaino since `[SPEC-SUI-135]`'s handoff, so `review.js` runs on Vaino's own origin; a route on the console's separate process would be a cross-origin call the browser refuses without CORS the console has no reason to grow. The rate limiter lives as a process-wide mutex in `player/src/web/musicbrainz.rs` (`web.rs` at the time, split 2026-09-02), gated `sampo-support` along with the rest of this page, and Vaino's own `reqwest` dependency is declared `optional`, wired to that same feature, so an appliance build never resolves or compiles an HTTP client it will never call.
...
**Recording/release/artist/track are used in the precise senses [SPEC023](SPEC023-domain-vocabulary.md) defines.** Choosing among "the releases Sampo knows for it" two rows up is not always resolving a real ambiguity: MusicBrainz routinely catalogues one physical/logical album as several release MBIDs (a US pressing, a European one, a digital reissue) that are functionally indistinguishable for every purpose here — same tracklist, same title. Differing release MBIDs do not, by themselves, mean two different albums; `release_recordings.chosen` records which catalogue entry got picked, not a claim that the underlying album is otherwise ambiguous (`[SPEC023]` Release).

**The release/track half of this table has a second caller now.** [SPEC028 §3](SPEC028-cd-ripping-identification.md#3-when-automation-isnt-enough--the-person-with-the-disc-decides) reuses `[SPEC-SUI-196]`'s search proxy for CD ripping's own manual-fallback path, and names the still-unbuilt release/track search as a second, independent reason to finish it — searching by *album title*, what a disc actually carries, rather than by one track's recording title alone.

The artist case was the one this table added, not merely extended: `recording_artists` is keyed `(mbid, artist_mbid)`, and nothing wrote to it outside Sampo's own ingest before this. A person confirming "this recording is right, but MusicBrainz's own credit is wrong" needed a decision `apply_reviews.py` could fold into `recording_artists` the same way it folds a recording reassignment into `passage_recordings` — a new decision shape, not a reuse of the existing one, because the row it corrects is a different table with a different key. Built 2026-08-27: `artist_reviews` carries `recording_mbid` and the previous credit (`previous_artist_mbid`/`name`/`weight`), captured at decision time for the same reason `id_reviews.previous_mbid` is — a credit correction has no re-derivable source the way a boundary edit does, so this is the only copy, and `apply_reviews.py --revert-artist` needs it to mean anything. Offered on any card whose stored id shape-checks as a real MusicBrainz id, independent of whatever else that card decides. See [IMPL006 Stage 10](../IMPL006-sampo-editing-workflows.md#stage-10--artist-only-correction) for what was verified.

**Designed and built 2026-08-27**, alongside `[REQ-LIB-175]`'s waveform editor — both requested together, both designed together, and the build order in [IMPL006](../IMPL006-sampo-editing-workflows.md) covers both, now complete.

---

**`[SPEC-SUI-199]` Built 2026-08-30. The queue's own narrowness was, by accident, the only door.** `review_queue` is deliberately CONTRADICTED-only — showing every passage nothing is wrong with would bury the real findings — but that also meant a passage that had simply never been fingerprinted, or whose stored id was merely not what someone wanted it to say, could not reach the search-and-reassign box (`[SPEC-SUI-196]`) at all, even though nothing about that box depends on being a finding. `Library::review_item_for(passage_id)` builds the same `ReviewItem` shape for one named passage regardless of `id_checks`, served at `GET /review/passage/:passage_id`; a never-checked passage gets a new `on-demand` severity (`SEVERITIES` in `player/src/db/library.rs`, `db.rs` at the time, split 2026-09-01) rather than a fabricated `checked_at`. `review.js`'s deep-link handling (`?passage=`) now asks this route directly whenever the passage was not already in the batch, so `profile.html`'s existing "Review in Vaino" handoff button works for any passage, not only ones the fingerprint pass flagged. No new write path — this reuses `record_review`/`apply_reviews.py` unchanged.
