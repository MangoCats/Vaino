...
**Status: designed, not yet built.** `tools/console.py`'s `scan()` currently reports only aggregate `assumed_here`/`changed`/`unclaimed`/`missing` counts, not this four-state per-file classification, and `/folder` offers no per-row induct/relink action — relink itself is a standalone binary (`player/src/bin/relink.rs`) not yet wired into the console at all. See `[SPEC-SUI-060]`'s `assumed`/`missing` output, which is what actually ships today.
...
> **Status.** The mechanics below — `[SPEC-SUI-140]`, `[SPEC-SUI-145]`, `[SPEC-SUI-150]`, `[SPEC-SUI-155]`, `[SPEC-SUI-170]` — were decided 2026-08-20 and **built 2026-08-27**: `ensure_vaino()` (`console.py` at the time, split into `tools/vaino_control.py`), and `review.js`'s `?passage=` deep link. `[SPEC-SUI-135]`'s waveform editor is [built too](SPEC021-waveform-boundary-editor.md) as of the same date. **Correction, found building `[REQ-LIB-190]`:** the profile page had only ever grown the "Review in Vaino" box; the editor's own handoff was designed and its mechanics existed, but nothing on the page linked to `/edit/:passage_id` at all. `handoffBox()` is now parameterised by route and label, called twice — id review and the waveform editor are the same handoff shape aimed at a different route, not two mechanisms.
