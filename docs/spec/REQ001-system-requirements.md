...
- **`[REQ-UI-020I]` Grid View Pagination & Diacritic Normalization**: Both Artists View and Albums View MUST provide full pagination controls (Next/Prev buttons, page indicators) with a default limit of 100 items per page to prevent catalog truncation. Artist sort names MUST normalize diacritics/accents (e.g. `Mötley Crüe` $\rightarrow$ `Motley Crue`) so that accented artists appear under their ASCII letter key (`M`).
