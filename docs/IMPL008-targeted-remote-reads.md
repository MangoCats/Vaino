...
**The read itself, per `[SPEC-DF-116]`:** vainopi already carries the `sqlite3` CLI (confirmed live: 3.40.1, `-json` output works — `ssh pi@vainopi "sqlite3 -json /var/vaino/listener.db '...'"` returns a JSON array directly — **path corrected 2026-09-12**: vainopi split its database on 2026-09-10 and `/srv/library/vaino.db` is now a 0-byte leftover, so the original path would have returned nothing and said nothing. The anchors named below resolve in the listener half; catalogue reads want `--library`'s `/srv/library/library.db` instead). Use it for a **per-item** query instead of a database copy: when Sampo's profile page for one passage/recording is opened, fetch just that item's current remote value via the identical anchors `apply_changes.py` already resolves locally (`audio_md5`+kind+start/end for a boundary, `recording_mbid` for an artist/id review) — not a copy, a handful of bytes.
